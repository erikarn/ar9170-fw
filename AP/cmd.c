...
#if 0 /* ART/MDK no need on normal driver */
...
#if 0
        zfUartSendStrAndHex((u8_t*)"ZM_CMD_EKEY done: user=", setKeyCmd->user);
        zfUartSendStrAndHex((u8_t*)"key ID=", setKeyCmd->keyId);
        zfUartSendStrAndHex((u8_t*)"EncType=", setKeyCmd->type);
        zfUartSendStrAndHex((u8_t*)"mac[0]=", setKeyCmd->macAddr[0]);
        zfUartSendStrAndHex((u8_t*)"mac[1]=", setKeyCmd->macAddr[1]);
        zfUartSendStrAndHex((u8_t*)"mac[2]=", setKeyCmd->macAddr[2]);
...
void zfMemoryCopyInWord(u32_t* dst, u32_t* src, u32_t byteCount)
{
    u32_t i;
...
{
    u32_t usb_mode_ctrl;
    u32_t usb_max_agg_num;
    u32_t usb_agg_timeout;
