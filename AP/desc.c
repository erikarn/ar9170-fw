...
/*                                                                      */
/*    NOTES                                                             */
/*      LastAddr tempary point (same) to nextAddr after initialize.     */
...
#if ZM_INT_USE_EP2 == 1
{
	...
	{
	    *(volatile u32_t*)(ZM_RSP_BUFFER + (i*4)) = 0xffffffff;
	}
}
#endif
	...
	*(volatile u32_t*)(ZM_BA_BUFFER + 4) = 0x0026c401;
	...
	/* softwap */
	zgDnQCurrentDescPtr = zgDnQ.head;
    ...
    zfUartSendStrAndHex((u8_t*)"ZM_RX_BLOCK_NUMBER   =", ZM_RX_BLOCK_NUMBER);
    zfUartSendStrAndHex((u8_t*)"ZM_TX_BLOCK_NUMBER   =", ZM_TX_BLOCK_NUMBER);
...
/************************************************************************/
/*                                                                      */
/*    FUNCTION DESCRIPTION                  zfQuePutPacket              */
/*      Put a complete packet into the tail of the Queue q.             */
/*                                                                      */
/*    ROUTINES CALLED                                                   */
/*                                                                      */
...
/*                                                                      */
/*    AUTHOR                                                            */
/*      Wade Sung   Atheros Communications, Inc.                2007.12 */
/*                                                                      */
/*    NOTES                                                             */
/*                                                                      */
/************************************************************************/
...
//	zfUartSendStrAndHex((u8_t*)"qp = ", (u32_t)desc);
    ...
    {
        /* put to tail */
        q->terminator->lastAddr->nextAddr = desc;
        q->terminator = desc;
    }
    else
    {
...
/************************************************************************/
/*                                                                      */
/*    FUNCTION DESCRIPTION                  zfQueGetPacket              */
/*      Get a complete packet from the head of the Queue q.             */
/*                                                                      */
/*    ROUTINES CALLED                                                   */
/*                                                                      */
/*    INPUTS                                                            */
...
u32_t zfLengthConfirm(struct zsDmaDesc* desc)
{
	...
	if ( desc_len > (ctrl_len+4) )
	{
		if (!((ctrl_len+4) & 0x1ff)) //? only debug
		{
...
/* check the aggregation push bit */
/* return 1 : push bit on         */
/* return 0 : push bit off        */
u8_t zfCheckAGP(struct zsDmaDesc* desc)
{
    u8_t ret = 0;
    u16_t* buf;
...
/* check the aggregation bit         */
/* return 0 : not aggregation packet */
